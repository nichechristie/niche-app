...
import { BibleLesson, Question } from "@/types/bible";
...
export function BibleStudy() {
  ...
  const loadProgress = async () => {
    ...
    try {
      const response = await fetch(`/api/bible/progress?address=${address}`);
      if (response.ok) {
  ...
  const startLesson = (lesson: BibleLesson) => {
  ...
  if (!address) {
    return (
      <div className="glass rounded-xl p-8 text-center">
        <h2 className="text-2xl font-bold text-white mb-4">Bible Study</h2>
        <p className="text-gray-400">Please connect your wallet to access Bible lessons and earn NICHE tokens.</p>
      </div>
    );
  }
          ...
          <div className="flex gap-4">
            {passed && !isLessonCompleted(selectedLesson.id) && (
              <button
                ...
                disabled={claiming}
                className="flex-1 px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white rounded-lg font-semibold transition-all disabled:opacity-50"
              >
    ...
    return (
      <div className="glass rounded-xl p-8 max-w-3xl mx-auto">
        ...
        {currentQuestion === 0 && answers.length === 0 && (
          <div className="mb-6 p-6 bg-gradient-to-br from-indigo-500/20 to-purple-500/20 rounded-xl border border-indigo-500/30">
            <p className="text-sm text-indigo-300 mb-2">Scripture: {selectedLesson.scripture}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="glass rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">Bible Study</h1>
            <p className="text-gray-400">Learn God's Word and earn NICHE tokens</p>
          </div>
          <div className="text-right">
            <p className="text-sm text-gray-400">Total Earned</p>
            <p className="text-2xl font-bold text-green-400">{totalRewards} NICHE</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="glass-inner rounded-lg p-4 text-center">
            <p className="text-2xl font-bold text-white">{bibleLessons.length - completedLessons.length}</p>
            <p className="text-sm text-gray-400">Remaining</p>
          </div>
          <div className="glass-inner rounded-lg p-4 text-center">
            <p className="text-2xl font-bold text-white">{bibleLessons.length}</p>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <p className="text-sm text-indigo-400">📖 {lesson.scripture}</p>
                <p className="text-lg font-bold text-green-400">+{lesson.reward} NICHE</p>
              </div>
