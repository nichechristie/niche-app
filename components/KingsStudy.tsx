...
export function KingsStudy() {
  const { address } = useAccount();
  const [selectedLesson, setSelectedLesson] = useState<KingsLesson | null>(null);
  ...
  const loadProgress = async () => {
    ...
    try {
      const response = await fetch(`/api/kings/progress?userId=${address}`);
      if (response.ok) {
  ...
  const startLesson = (lesson: KingsLesson) => {
  ...
  const completeLesson = async () => {
    ...
    setClaiming(true);
    try {
      const response = await fetch("/api/kings/complete", {
        method: "POST",
  ...
  if (!address) {
    return (
      <div className="glass rounded-xl p-8 text-center">
        ...
        <p className="text-gray-400">Please connect your wallet to access Kings history lessons and earn NICHE tokens.</p>
      </div>
    );
  }
        ...
        {currentQuestion === 0 && answers.length === 0 && (
          <div className="mb-8">
            <div className="bg-indigo-500/20 border border-indigo-500 rounded-lg p-6 mb-6">
              <p className="text-indigo-300 font-semibold mb-2">{selectedLesson.period}</p>
    ...
    <div className="glass rounded-xl p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-400">
          Learn about the kings of Israel and Judah, from the united monarchy to the divided kingdom.
        ...
        {totalRewards !== "0" && (
          <div className="mt-4 bg-green-500/20 border border-green-500 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-300">
              Completed: {completedLessons.length} of {kingsLessons.length} lessons
            </p>
          </div>
        )}
      </div>
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-indigo-400 mb-2">{lesson.period}</p>
